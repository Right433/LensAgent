...
def rms_calculator(lens_idx: str) -> str:
    ...
    if not ALL_LENSES and lens_idx >= len(ALL_LENSES):
        return "镜头数据未加载"
...
7. **Zemax 验收**: zemax_optimize 返回带【RMS 数值比较】结论（✅达标/❌未达标），直接按结论走。
   ...
   ★ 达标 → 调 zemax_layout(lens_idx=X) → 调 get_spot_diagram(lens_idx=X) → 调 save_zmx(lens_idx=X) → Final Answer。
     zemax_layout 默认保存到 /gz-data/layout_{{lens_idx}}.png，save_zmx 默认保存到 C:/zemax_results/lens_{{lens_idx}}.zmx
...
Final Answer 格式:
Final Answer: 镜头#xxxxx | FOV=xx° F/x.x | RMS=x.xxxx mm | 结论: 达标✓  (或 未达标✗)
...
ZEMAX_BRIDGE = _os_for_bridge.environ.get("ZEMAX_BRIDGE_URL", "https://scruffy-unsubtly-grandma.ngrok-free.dev")
            ...
            if rms_target is not None:
                ...
                if rms_pass:
                    msg += (
                        ...
                        f"  Step4: Final Answer: 镜头#{lens_idx} | FOV=<度> F/<F#> | RMS={rms_worst:.4f}mm | 结论: 达标✓\n"
    ...
    try:
        ...
        for seg in str(input_str).strip().split(","):
            ...
            if "=" in seg:
                ...
                parts[k.strip()] = v.strip()
        lens_idx  = int(parts.get("lens_idx", 0))
        # 默认文件名加时间戳，避免多 case 互相覆盖
        import time as _ts_zmx
        _ts = _ts_zmx.strftime("%Y%m%d_%H%M%S")
        save_path = parts.get("save_path", f"C:\\zemax_results\\lens_{lens_idx}_{_ts}.zmx")
    except Exception as e:
        return f"输入格式错误: {e}"

...
llm = ChatOpenAI(
    ...
    temperature=0,
    max_tokens=512,    # qwen3-14b 上限8192，严格控制
)
...
def rank_by_rms(query: str) -> str:
    ...
    if fallback:
        out["warning"] = (f"数据库中无FOV≈{fov_target}° F/{fnum_target}的镜头，"
                          f"以下为最近邻结果，FOV/Fnum可能差距较大，建议降低要求或重新描述需求。")
...
def random_restart(input_str: str) -> str:
    """
    【随机重启】优化停滞时，对镜头关键面施加随机扰动，跳出局部极值后重新优化。
    输入格式: "lens_idx=<编号>, strength=<扰动强度0.01-0.10>"
      - strength=0.03 表示对radius施加±3%随机扰动（推荐起始值）
      - strength=0.08 表示更大扰动，用于深度停滞
    示例: "lens_idx=36237, strength=0.05"
    注意: 扰动后必须调用 rms_calculator 查看新RMS，判断是否跳出局部极值。
          若RMS更差可调用 reset_lens 恢复原始参数。
    """
    import random, math
    try:
        input_str = str(input_str).strip().strip(chr(34)).strip(chr(39))
...
def local_optimize(input_str: str) -> str:
    ...
    best_rms      = rms_init
    no_improve    = 0
    effl_tol      = 0.02          # EFFL 漂移容忍度 ±2%
    rms_floor     = 0.005         # RMS 下限，避免近轴过拟合
    effl_penalty  = 50.0          # EFFL 超限时的惩罚权重，加强约束
    ...
    def penalized_rms(result):
        ...
        penalty = effl_penalty * max(0.0, effl_drift - effl_tol)
        return rms + penalty
...
Step0b rank_by_rms时加fov_tol/fnum_tol过滤，如: FOV=75度 F/2.4, fov_tol=5, fnum_tol=0.5；若返回warning说明数据库无此范围，告知用户并用最近邻继续
...
⚠️ Action格式必须严格如下，Action和Action Input分两行，Action Input只写参数值不写函数名：
Thought: 分析
...
⚠️ 达到目标或完成判断后，必须立即输出 Final Answer，禁止继续调用工具：
Thought: 已达标，结束
...
def build_agent():
    tools = [
        ...
        local_optimize,
        split_lens,
        interpret_requirement,  # 新增：自然语言需求翻译
    ]
